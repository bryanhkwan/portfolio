...
The reused renderer has a small, documented portfolio adaptation: a 30 FPS rendering budget, DPR 1, no multisampling or shadow maps, and redraw only when the scene changes. This reduces graphics work without changing the original Blender models, poses, or public replay data. Original and adapted module hashes are recorded in `public/courtvision/provenance.json`.
